...
if ($report->criteriasValidated()) {
   ...
   $report->setColumns(array(
	   ...
	   , new PluginReportsColumnInteger('delay_close', __('Delai de fermeture'))
	   , new PluginReportsColumn('delay_solve', __('Delai de resolution'))
	   , new PluginReportsColumn('account1', __('Delai de prise en charge'))
	   , new PluginReportsColumn('actiontime1', __('Duree action'))
   ));
	...
	, ROUND(glpi_tickets.close_delay_stat/3600, 2) as delay_close
	, ROUND(glpi_tickets.solve_delay_stat/3600,2) as delay_solve
	, ROUND(glpi_tickets.takeintoaccount_delay_stat/3600, 2) as account1
	, ROUND(glpi_tickets.actiontime/3600,2) as actiontime1
	FROM `glpi_tickets` 
