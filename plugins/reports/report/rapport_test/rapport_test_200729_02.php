...
echo "fffffff: $query02";		
#	$reponse02 = $DB->query($query02);
	$reponse02 = $DB->request($query02)->next();
echo "ttttttttttttttttttttttttt";	
	
	$span_lig=0;
	$Span = array();
	for($i=0;$i<$DB->numrows($reponse02);$i++)    {
		$data02=$DB->fetch_row($reponse02);
		$vCat=$data02['type'];
...
echo "uuuuuuuuuuuuuuuuuuuuuuuuuuu";
	...
	if ($span_lig > 0 ) {
		...
		echo"<tbody>";
		$reponse = $DB->request($query)->next();
		$Sum_Temps_moy=0;
		$Sum_Nb_ticket=0;
		$Type_02="";
		$_SESSION['dataExport']=array();
		for($i=0;$i<$DB->numrows($reponse);$i++)    {
			$data=$DB->fetch_row($reponse);
			$Type = $data['Type'];
			$Cat = $data['Catégorie'];
		...
		if ($Sum_Nb_ticket > 0) {
			...
			echo "<tfoot><tr><th> Total</th><td></td><td></td><td>$Sum_Nb_ticket</td><td>$Sum_Nb_ticket</td></tr></tfoot>";
		};	
		echo "</table></center>";
